...
for i in range(1,4):
    ...
    for sub in range(len(ID)):
        ...
        Ceff_sub[sub], sigma_sub[sub], a_sub[sub], FCemp_sub[sub], FCsim_sub[sub], error_iter_sub_aux, errorFC_iter_sub_aux, errorCOVtau_iter_sub_aux = \
                                            ...
                                            epsFC_Ceff=epsFC_Ceff/100, epsCOVtau_Ceff=epsCOVtau_Ceff/100, epsFC_sigma=epsFC_sigma*10, epsCOVtau_sigma=epsCOVtau_sigma*10,
                                            MAXiter=MAXiter+10000, error_tol=error_tol, patience=patience+1.5, learning_rate_factor=learning_rate_factor,
                                            Ceff_norm=Ceff_norm, maxC=maxC,
                                            iter_check=iter_check, plot_evol=False, plot_evol_last=False)
