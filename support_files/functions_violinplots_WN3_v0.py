...
def plot_violins_HC_MCI_AD(ax, data,
                 font_scale=1.4,
                 point_size=3,  # Decreased marker size by half
                 metric='Metric?',
                 plot_title='Title?',
                 xgrid=False,
                 saveplot=False,
    ...
    sns.violinplot(y="value",
                   ...
                   ax=ax,
                   zorder=1,
                   dodge=False)  # Ensure alignment with swarmplot

    # Swarmplot overlay
    sns.swarmplot(y="value",
                  x="cond",
                  data=data,
                  s=point_size,  # Updated marker size
                  palette=swarmplot_palette,
                  hue="cond",
                  legend=False,
                  ax=ax,
                  zorder=1,
                  dodge=False)  # Ensure alignment with violinplot
    ...
    for i, (group1, group2) in enumerate(comparisons):
        ...
        vals1 = data[data['cond'] == group1]['value'].dropna()
        vals2 = data[data['cond'] == group2]['value'].dropna()
        ...
        y = base_y + i * (height / 2)  # Reduce height by half
        ax.plot([x1, x1, x2, x2], [y - 0.0002, y + 0.0005, y + 0.0005, y - 0.0002], lw=1.5, color='black')  # Adjust line height
        ax.text((x1 + x2) / 2, y + 0.0006, asterisks if p_value < 0.05 else f'p = {p_value:.3f}', 
